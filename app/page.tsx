...
import { getTankerWatchDashboardData } from "../lib/tanker-watch/source";
...
export default async function HomePage() {
  ...
  const tankerWatchData = await getTankerWatchDashboardData();
  const screenSnapshot = formatScreenSnapshot();
  const tankerRoutes = tankerWatchData.routes;
  const soonestTanker = tankerWatchData.soonestRoute;
  const bufferMetrics: BufferMetric[] = [
    { label: "Storage cover", value: "30d", tag: "Amber", status: "warning" },
    {
      label: "Tracked inbound",
      value: `${tankerRoutes.length}`,
      tag: tankerWatchData.sourceMode === "live" ? "Live" : "Fallback",
      status: "flat",
    },
    {
      label: "Next berth",
      value: soonestTanker.etaBadge,
      tag: soonestTanker.destination,
      status: "up",
    },
        ...
        <div className="dashboard-grid">
          ...
          <section className="panel span-8">
            <div className="panel-header">
              ...
              <span className="market-tag is-warning">{tankerWatchData.providerBadge}</span>
            </div>

            <div className="tanker-layout">
              <div className="map-stage">
                <div className="map-caption">
                  {tankerWatchData.providerLabel} tracking across{" "}
                  {tankerWatchData.trackedPortCount} NZ fuel ports
                </div>
